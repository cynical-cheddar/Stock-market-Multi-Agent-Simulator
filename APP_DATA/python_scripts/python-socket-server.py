...
import Traders.MessageConstants as msg_consts
...
def HandleIncomingCommand(msg_dict):
    ...
    if(commandType == comms.CommandType.NewActiveStatus or commandType == comms.CommandType.NewRole):
...
lasttime = time.time()
lasttime_throttle = time.time()
throttleRate = 100

outgoing_message_queue = Queue(maxsize = 99999)
...
while True:
    ...
    if(time.time() - lasttime_throttle > (1/throttleRate)):
        if(not outgoing_message_queue.empty()):
            message = outgoing_message_queue.get()
            unity_socket.SendData(message) # Send this string to other application
            lasttime_throttle = time.time()
    # ping unity once per 2 seconds
    if(time.time() - lasttime > 2):
        lasttime = time.time()
        # define a ping

        ping = json.dumps(comms.OutgoingRequestMessage(messageType= comms.MessageType.Request, source_pid=msg_consts.ZMQ_SERVER_PID, target_pid=msg_consts.UNITY_PID,  source_tid=0, target_tid="", dataString="ping", requestType= comms.RequestType.Ping).__dict__)
